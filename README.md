...
- по умолчанию запрашивает права администратора при запуске (один запрос пароля на сессию запуска);
...
> [!IMPORTANT]
> При старте приложение запрашивает авторизацию администратора (`pkexec`, fallback `sudo`),  
> чтобы дальше выполнять системные операции без постоянных повторных запросов пароля.

## Установка через deb

После установки пакета запускайте приложение командой:

```bash
zapret-for-ubuntu
```

или из меню приложений (`Zapret for Ubuntu`).
...
- на каждый push в `master` собирается `.deb`;
- пакет и `.sha256` загружаются в artifacts;
- в релизе `deb-latest` ассеты автоматически заменяются свежей сборкой.
...
- кнопка `Remove` в GUI:
  - удаляет `zapret-discord-autostart.service`;
  - удаляет директорию `zapret-discord`;
  - удаляет приложение из системы (если установлено как пакет).
- скрипт:
  - `./uninstall-zapret.sh`
  - можно запускать вручную, при необходимости запросит права администратора.
